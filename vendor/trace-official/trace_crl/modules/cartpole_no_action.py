"""Temporal VAE with gaussian marginal and laplacian transition prior"""
"""Simplified version: No action branching"""
...
class ModularShiftsNoAction(pl.LightningModule):
    """
    Simplified ModularShifts without action branching.
    Only uses domain embedding to condition the transition prior.
    """
    ...
    def __init__(
        ...
        correlation='Pearson'):
        '''Nonlinear ICA for general causal processes with modular distribution shifts'''
        super().__init__()
        ...
        # Single transition prior (no action branching)
        if trans_prior == 'L':
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.dyn_embedding_dim > 0:
            dyn_embeddings = self.dyn_embed_func(c)
        else:
            dyn_embeddings = None
            
        if self.obs_dim > 0 and self.obs_embedding_dim > 0:
        ...
        # Past KLD <=> N(0,1)
        p_dist = D.Normal(
            torch.zeros_like(mus[:, :self.lag, :self.dyn_dim]), 
        ...
        # Future KLD - Single transition prior (no action branching)
        ...
        if self.obs_dim > 0:
            p_dist_obs = D.Normal(
                obs_embeddings[:, :, 0].reshape(batch_size, length, 1), 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Only compare dyn_dim dimensions (CartPole state is 4D)
        zt_recon = mus[:, 0, :self.dyn_dim].reshape(-1, self.dyn_dim).T.detach().cpu().numpy()
        zt_true = batch["yt"][:, 0].reshape(-1, self.dyn_dim).T.detach().cpu().numpy()
